def train():
    ...
    # logging.info(f'After creation, Learning Rate: {scheduler.get_last_lr()[0]}')
    ...
    if path.exists(state_file):
        ...
        # logging.info(f'Before load_state, Learning Rate: {scheduler.get_last_lr()[0]}')
        if not online or state['config']['control']['online']: # if not online, will load the last lr using
            # logging.info(f'Before optimizer_load_state, Learning Rate: {scheduler.get_last_lr()[0]}')
            optimizer.load_state_dict(state['optimizer'])
            # logging.info(f'Between load_state, Learning Rate: {scheduler.get_last_lr()[0]}')
            scheduler.load_state_dict(state['scheduler'])
            # logging.info(f'After scheduler load_state, Learning Rate: {scheduler.get_last_lr()[0]}')
        scaler.load_state_dict(state['scaler'])
        best_perf = state['best_perf']
        steps = state['steps']
    # logging.info(f'After load_state, Learning Rate: {scheduler.get_last_lr()[0]}')
    ...
    def train_epoch():
        ...
        def read_json_with_multiple_encodings(file_path, encodings=['utf-8']):
            for encoding in encodings:
                try:
                    ...
                    continue
            raise UnicodeDecodeError(f"Failed to decode {file_path} with provided encodings.")

        # logging.info(f'At beginning of epoch, Learning Rate: {scheduler.get_last_lr()[0]}')
        player_names = []
        if online:
            ...
            if path.exists(file_index):
                ...
                if len(player_names_set) > 0:
                    ...
                    for filename in tqdm(file_list, unit='file'):
                        try:
                            start = read_json_with_multiple_encodings(filename)
                            if not set(start['names']).isdisjoint(player_names_set):
                                filtered.append(filename)
                        except UnicodeDecodeError as e:
                            logging.warning(f"Could not decode file {filename}: {e}")
                    file_list = filtered
                file_list.sort(reverse=True)
                torch.save({'file_list': file_list}, file_index)
            # player_names = ['FFToyGH', '九紋龍史進', '【罪歌】', '融', 'pawa', '神仙酵母', '凍魚凍魚', 'COEDO緑', '夏の夜の夢', 'zeRo', 'OMANTA', '夕凪', '一蓮峻潤', 'ASAPIN', '藤井聡太', '鳳南のぶたさん', '太くないお', '火時計を押せ！', 'お知らせ', '右折するひつじ', 'CLS', 'じょにおん！！', 'わっしょい君2号', 'いばらぎ', 'yoteru', 'トトリ先生19歳', 'vふーちゃんv', 'ウルトラ立直', '就活生@川村軍団', '全速ゆーへい']
            # player_names = ['CLS']
            # player_names = ['藤井聡太', '鳳南のぶたさん', '太くないお', 'yoteru', '全速ゆーへい']
        ...
        for obs, actions, masks, steps_to_done, kyoku_rewards, player_ranks in data_loader:
            ...
            if idx % opt_step_every == 0:
                if max_grad_norm > 0:
                    # logging.info(f'Before unscale, Learning Rate: {scheduler.get_last_lr()[0]}')
                    scaler.unscale_(optimizer)
                    # logging.info(f'After unscale, Learning Rate: {scheduler.get_last_lr()[0]}')
                    params = chain.from_iterable(g['params'] for g in optimizer.param_groups)
                    clip_grad_norm_(params, max_grad_norm)
                # logging.info(f'Before scale back, Learning Rate: {scheduler.get_last_lr()[0]}')
                scaler.step(optimizer)
                # logging.info(f'After scale back, Learning Rate: {scheduler.get_last_lr()[0]}')
                scaler.update()
                optimizer.zero_grad(set_to_none=True)
            # logging.info(f'Before step, Learning Rate: {scheduler.get_last_lr()[0]}')
            scheduler.step()
            # logging.info(f'After step, Learning Rate: {scheduler.get_last_lr()[0]}')
            pb.update(1)

            # current_lr = scheduler.get_last_lr()[0]
            # logging.info(f'Step: {steps}, Learning Rate: {current_lr}')
            # logging.info(f'Step: {steps}, learning rate: {scheduler.get_last_lr()[0]}')
            # writer.add_scalar('learning_rate', current_lr, steps)

            # # Print learning rate for debugging
            # print(f'Step: {steps}, Learning Rate: {current_lr}')
