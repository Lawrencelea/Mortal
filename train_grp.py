...
class GrpFileDatasetsIter(IterableDataset):
    ...
    def build_iter(self):
        while True:
            ...
            for start_idx in range(0, len(self.file_list), self.file_batch_size):
                # logging.info(f"Populating buffer from index {start_idx}")
                self.populate_buffer(start_idx)
                buffer_size = len(self.buffer)
                # logging.info(f"Buffer size after population: {buffer_size}")
    ...
    def populate_buffer(self, start_idx):
        ...
        for file_name in file_list:
            try:
                data = Grp.load_gz_log_files([file_name])
                for game in data:
                ...
                logging.error(f"Error processing file {file_name}: {e}. Skipping file.")
                if os.path.exists(file_name):
                    os.remove(file_name)
                    logging.info(f"Deleted state file in finally block: {file_name}")
...
def train():
    ...
    for inputs, rank_by_players in train_data_loader:
        # logging.info(f"Processing batch {steps}")
